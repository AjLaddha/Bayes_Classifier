...
class GaussNB:
    ...
    def cov_matrix_avg(self,var1,var2,var3):
    	
        cov_matrix=(var1+var2+var3)/3
    ...
    def g(self,mean,cov,p):
        ...
        w0=w0+math.log(p)
        return [w,w0]
    def boundary_decision_two_classes(self,mean1,mean2,cov,x):
        v= cov[0]
        w1=(np.transpose(mean1-mean2))/v
        x12=(mean1+mean2)/2
        g12=np.dot(w1,x-x12)
        if g12>=0:
            return '1'
            ...
            return '0'
    def boundary_decision(self,mean1,mean2,mean3,cov,p1,p2,p3,x):
        w1,w01=self.g(mean1,cov,p1)
        w2,w02=self.g(mean2,cov,p2)
        w3,w03=self.g(mean3,cov,p3)
        w=np.transpose(w1-w2)
        w0=w01-w02
        g12=np.dot(w,x)
        g12=g12+w0
        w=np.transpose(w2-w3)
        w0=w02-w03
        g23=np.dot(w,x)
        g23=g23+w0
        w=np.transpose(w3-w1)
        w0=w03-w01
        g31=np.dot(w,x)
        g31=g31+w0
...
def main():
        ...
        p1=(len(train_list1)*1.0)/(len(train_list1)+len(train_list2)+len(train_list3))
        p2=(len(train_list2)*1.0)/(len(train_list1)+len(train_list2)+len(train_list3))
        ...
        for i in range(len(test_list1)):
            class_identified = nb.boundary_decision(mean1,mean2,mean3,cov_matrix,p1,p2,p3,test_list1[i])
        ...
        for i in range(len(test_list2)):
            class_identified=nb.boundary_decision(mean1,mean2,mean3,cov_matrix,p1,p2,p3,test_list2[i])
            if class_identified=='C2':
                true_pred_c2=true_pred_c2+1
                conf_matrix[4]=conf_matrix[4]+1
            elif class_identified=='C1':
                conf_matrix[3]=conf_matrix[3]+1
            elif class_identified=='C3':
                conf_matrix[5]=conf_matrix[5]+1
            my_dict[class_identified].append([test_list2[i],'C2'])
        

        for i in range(len(test_list3)):
            class_identified=nb.boundary_decision(mean1,mean2,mean3,cov_matrix,p1,p2,p3,test_list3[i])
            if class_identified=='C3':
                true_pred_c3=true_pred_c3+1
                conf_matrix[8]=conf_matrix[8]+1
            elif  class_identified=='C1':
                conf_matrix[6]=conf_matrix[6]+1
            elif class_identified=='C2':
                conf_matrix[7]=conf_matrix[7]+1
        ...
        mean_prec=(prec_c1+prec_c2+prec_c3)/3
        mean_recall=(recall_c1+recall_c2+recall_c3)/3
        f_mean=(2.0*mean_prec*mean_recall)/(mean_prec+mean_recall)
        print true_pred_c1 , true_pred_c2 ,  true_pred_c3
        accuracy=((true_pred_c1+true_pred_c2+true_pred_c3)*1.0)/(len(test_list1)+len(test_list2)+len(test_list3))
        accuracy_12=((true_pred_c1+true_pred_c2)*1.0)/(len(test_list1)+len(test_list2))
        accuracy_23=((true_pred_c2+true_pred_c3)*1.0)/(len(test_list3)+len(test_list2))
        accuracy_31=((true_pred_c3+true_pred_c1)*1.0)/(len(test_list1)+len(test_list3))
        ...
        print np.reshape(conf_matrix,(3,3))
        print "Accuracy for class 1 &2 ", accuracy_12
        print "Accuracy for class 2 &3 ", accuracy_23
        print "Accuracy for class 3 &1 ", accuracy_31

	# # code snippet to plot decision regions


        d1_x = dataset1[:, 0]
        d2_x = dataset2[:, 0]
        d3_x = dataset3[:, 0]
        all_x = np.concatenate((d1_x, d2_x, d3_x), axis=0)
        d1_y = dataset1[:, 1]
        d2_y = dataset2[:, 1]
        d3_y = dataset3[:, 1]
        all_y = np.concatenate((d1_y, d2_y, d3_y), axis=0)
        ...
        for i in drange2(int(min_y-1), int(max_y+1), 0.05):
        	for j in drange2(int(min_x-1), int(max_x+1), 0.05):
        		...
        		temp.append(j)
        		temp.append(i)
        		class_identified = nb.boundary_decision(mean1, mean2, mean3, cov_matrix,p1,p2,p3,temp)
        		Z = -1
        		if class_identified == "C1" :
        			a.append(temp)
        		elif class_identified == "C2":
